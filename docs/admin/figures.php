...
function rd_show_figures()
{
    ...
    while (false !== ($rows = $db->fetchArray($result))) {
        ...
        $figures[] = [
            ...
            'text' => mb_substr(strip_tags($fig->getVar('desc')), 0, 150) . (mb_strlen($fig->getVar('desc')) <= 150 ? '' : '...'),
        ];
    }
...
function rd_figures_form($edit = 0)
{
    ...
    $ele = new RMFormSelect(__('Alignment', 'docs'), 'align', 0, $edit ? [ $fig->getVar('align') ] : ['center']);
...
function rd_locate_figure()
{
    ...
    $counter = 0;
    while (false !== (list($fig) = $db->fetchRow($result))) {
        $counter++;
        if ($fig == $id) {
