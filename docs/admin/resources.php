...
function rd_show_form($edit = 0)
{
    ...
    if ($edit) {
        $form->addElement(new RMFormUser(__('Document owner', 'docs'), 'owner', 0, $edit ? [$res->getVar('owner')] : '', 30));
    }
    ...
    $form->addElement(new RMFormGroups(__('Groups that can see this Document', 'docs'), 'groups', 1, 1, 5, $edit ? $res->getVar('groups') : [1, 2]), true);
...
function public_resources($pub = 0)
{
    ...
    $resources = rmc_server_var($_POST, 'ids', []);
    $page = rmc_server_var($_POST, 'page', 1);
...
function approved_resources($app = 0)
{
    ...
    foreach ($resources as $k) {
        ...
        if (!$res->save()) {
            ...
            if ($app && !$approved) {
                $errors .= true !== RDFunctions::mail_approved($res) ? __('Notification email could not be sent!', 'docs') . '<br>' : '';
            }
        }
    }
