...
function formPublish()
{
    ...
    // Licenses
    $select = new RMFormSelect(__('License', 'docs'), 'license', 0, $res->isNew() ? null : [$res->getVar('license')]);
    ...
    $form->addElement(new RMFormUser(__('Editors', 'docs'), 'editors', 1, $res->isNew() ? ($xoopsUser ? [$xoopsUser->uid()] : []) : $res->editors, 30));

    //Grupos con permiso de acceso
    $form->addElement(new RMFormGroups(__('Groups that can read Document', 'docs'), 'groups', 1, 1, 1, $res->isNew() ? [1, 2] : $res->groups), true);
